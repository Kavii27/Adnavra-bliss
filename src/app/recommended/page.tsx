...
export async function generateMetadata() {
  ...
  return {
    title: t("cat.metaRecTitle"),
    description: t("cat.metaRecDesc"),
  };
}
    ...
    <main className="relative min-h-screen bg-[#FDF9F3]">
      ...
      <section className="px-6 lg:px-12 pt-10 pb-16 max-w-[1400px] mx-auto">
        ...
        <p className="mt-2 text-sm text-[#8A8377]">
          {businesses.length} {businesses.length === 1 ? t("cat.one") : t("cat.other")} {t("cat.recForYou")}
        </p>

        <div className="mt-8">
          <VenueGrid
            businesses={businesses}
            emptyText={t("cat.emptyRec")}
          />
        </div>
      </section>
