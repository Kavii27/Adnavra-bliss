      ...
      <div className="max-w-2xl">
        ...
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-lg bg-white flex items-center justify-center border border-[#E9E1D3]">
            <Clock className="h-5 w-5 text-[#9A7B4F]" />
          </div>
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.22em]" style={{ color: "#9A7B4F" }}>
              Settings
            </p>
            <h1 className="font-[family-name:var(--font-display)] mt-0.5 text-2xl font-medium tracking-tight text-[#1F1B17]">Scheduling</h1>
            <p className="text-sm text-[#8A8377] mt-1">Set opening hours for each day. Closed days are not bookable.</p>
          </div>
        </div>
        ...
        {!businessId ? (
          <div className="mt-6 rounded-2xl border border-[#E9E1D3] bg-white shadow-[0_4px_20px_rgba(30,28,26,0.05)] p-8 text-center">
            <p className="text-sm text-[#8A8377]">No business found. Create your profile in Business setup first.</p>
            <Link href="/dashboard/settings/business" className="mt-4 inline-flex items-center justify-center rounded-lg bg-white px-4 py-2 text-sm font-semibold text-[#1F1B17] hover:bg-[#FBF7EF]">
              Go to Business setup
            </Link>
          </div>
        ) : (
          <div className="mt-6 rounded-2xl border border-[#E9E1D3] bg-white shadow-[0_4px_20px_rgba(30,28,26,0.05)] p-6 space-y-4">
            <div className="grid gap-2">
              {DAYS.map((d) => (
                <div key={d} className="flex items-center gap-3 rounded-lg bg-[#FAF7F2] border border-[#E9E1D3] px-3 py-2.5">
                  <span className="capitalize text-sm w-24 font-medium text-[#1F1E1D]">{d}</span>
                  <label className="flex items-center gap-1.5 text-xs text-[#8A8377] cursor-pointer">
                    <input
                      type="checkbox"
                      checked={hours[d].closed}
                      onChange={(e) => setHours((h) => ({ ...h, [d]: { ...h[d], closed: e.target.checked } }))}
                      className="accent-[#795831]"
                    />{" "}
                    Closed
                  </label>
                  {!hours[d].closed && (
                    <>
                      <Input
                        ...
                        type="time"
                        className="h-8 max-w-[120px] bg-[#FBF7EF] border-[#E9E1D3] text-[#1F1E1D]"
                      />
            ...
            <Button onClick={handleSave} disabled={saving} className="w-full rounded-full bg-[#1F1B17] text-white text-xs font-bold uppercase tracking-[0.12em] hover:bg-[#795831]">
              {saving ? (
                <>
            ...
            <p className="text-xs text-center text-[#8A8377]">
              ...
              <Link href="/dashboard/settings/business" className="text-[#1F1E1D] hover:underline">
                Business setup
              </Link>
              .
            </p>
