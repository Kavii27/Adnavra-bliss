...
import { createTimesheetSchema, updateTimesheetSchema } from "@/schemas/timesheet";
import {
  checkRateLimit,
...
export async function GET(request: NextRequest) {
  ...
  const [data, total] = await Promise.all([
    db.timesheetEntry.findMany({ where, include: includeRel, orderBy: { date: "desc" }, skip, take: limit }),
...
export async function POST(request: NextRequest) {
  ...
  const rate = await checkRateLimit(`timesheet:${ctx.ip}`, 60);
  ...
  if (parsed.data.clockIn && parsed.data.clockOut && parsed.data.clockOut <= parsed.data.clockIn) {
    return NextResponse.json({ error: "clockOut must be after clockIn" }, { status: 400 });
  }

  const entry = await db.timesheetEntry.create({
    data: { ...rest, businessId: effective },
    include: includeRel,
  });
  ...
  await auditLog({
    ...
    metadata: { staffMemberId: entry.staffMemberId, hours: entry.hours },
    ip: ctx.ip,
  });

  return NextResponse.json({ data: entry }, { status: 201, headers: rate.headers });
}
...
export async function PATCH(request: NextRequest) {
  ...
  const existing = await db.timesheetEntry.findUnique({ where: { id } });
  ...
  const updated = await db.timesheetEntry.update({ where: { id }, data: parsed.data, include: includeRel });
