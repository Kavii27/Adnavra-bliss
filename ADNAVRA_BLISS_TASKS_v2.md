# ADNAVRA BLISS — Mobile-First Rebuild Task List (v2 — full code, no shorthand)

**v2 note:** every spot in the previous version that said "unchanged",
"same pattern as before", "no code change needed", or showed a
`/* comment */` instead of real code has been replaced with the actual,
verbatim current file content plus the actual, complete new code — pulled
directly from the uploaded codebase, not paraphrased or assumed. Where a
list of items (categories, salon types) can grow over time, there is now
an explicit numbered "how to add a new one later" section right after it.
If your agent ever hits a task that still only *describes* a change instead
of showing the exact code, stop and ask a human — don't guess.
...
`src/lib/categories.ts` already defines **exactly 8** `SERVICE_CATEGORIES`.
This is the exact current content of that array — this is what the file
looks like *right now*, copied verbatim, not paraphrased:
```ts
// src/lib/categories.ts (current SERVICE_CATEGORIES block, top of the file — verbatim)
...
```
This same file **already has** `BUSINESS_TYPES` defined (see 2.2.a below —
just without icons yet) and two helper functions your agent will see used
throughout the codebase and should **not** recreate: `getCategoryLabel(slug)`
(returns the human label for any category-or-type slug) and
`isBusinessTypeSlug(slug)` (true if the slug belongs to `BUSINESS_TYPES`
rather than `SERVICE_CATEGORIES`). Both already exist — leave them as-is.
...
currently **never appears** (limit equals total, `8 < 8` is false). That's
fine functionally today (nothing to see-all to). **No code change is
required in this task (2.1) itself** — the checklist below is what to
literally do:
1. Open `src/lib/categories.ts` and confirm the array matches the block
   above (8 entries).
2. Open `src/app/page.tsx` and confirm `<BrowseByCategory limit={8} .../>`
   is called with `limit={8}` (search the file for `BrowseByCategory`).
3. Load the homepage at 375px and at 1440px and visually confirm the grid
   is `grid-cols-2` on mobile and `sm:grid-cols-4` on desktop, all 8 cards
   the same height, each with an icon + label. If it already looks like
   that, this task is done — do not add or remove anything.

### How to add a 9th (or 10th, 11th…) treatment category later — do this whenever a new category is needed
This exact same 3-step recipe applies **every time** a new category is
added in the future — the component code never needs to change again,
only this one array:
1. Open `src/lib/categories.ts`.
2. Pick a `lucide-react` icon (browse names at https://lucide.dev/icons) and
   add its name to the `import { ... } from "lucide-react";` line at the
   top of the file if it isn't already imported.
3. Add one new object to the `SERVICE_CATEGORIES` array, in the same shape
   as the existing entries, e.g.:
   ```ts
   { slug: "bridal-makeup", label: "Bridal makeup", icon: Gem },
   ```
   — `slug` must be lowercase, hyphenated, and unique; `label` is the exact
   text shown on the card; `icon` is the imported icon component (no quotes,
   no `()`  — pass the component itself).
That's it — no other file needs to change. `browse-by-category.tsx`,
`fetchVenuesByCategory`, the onboarding category step (6.1), and the
`/categories/[slug]` route all read from this same array automatically, so
adding a 9th entry here automatically makes the homepage show 8 cards +
a working "See all" link to the rest (since `limit={8} < 9` becomes true).
...
```
Leave the rest of the file (`ServiceCategorySlug`/`CATEGORY_SLUGS` types,
`BusinessTypeSlug`/`BUSINESS_TYPE_SLUGS`, `getCategoryLabel`,
`isBusinessTypeSlug`) exactly as it already is — only the two arrays
(`SERVICE_CATEGORIES` above, `BUSINESS_TYPES` here) get edited.

### How to add a new salon type later (e.g. an 8th type beyond the 7 above)
Same 2-step recipe as adding a category (2.1), applied to this array
instead:
1. Pick a `lucide-react` icon, add it to the `import { ... } from
   "lucide-react"` line at the top of `src/lib/categories.ts` if not
   already imported.
2. Add one object to `BUSINESS_TYPES`: `{ slug: "kebab-case-slug", label:
   "Display Label", icon: IconName }`. The `slug` must be unique across
   both `SERVICE_CATEGORIES` and `BUSINESS_TYPES` combined (they share the
   same lookup functions).
No other file changes — `BrowseBySalonType` (2.2.b), the onboarding salon
type step (6.2), and `fetchVenuesByCategory` (2.2.e) all read this array
directly, so a homepage `limit={8}` will keep showing 8 types + a working
"See all" the moment an 8th is added.
...
**2.2.e — Confirm the matching query already works — no edit, just verify
against this exact code.** This is the real, current, full function in
`src/lib/marketplace-venues.ts` — open the file and confirm it looks like
...
```
Because the `OR` already checks `salonTypes: { has: slug }`, calling this
same function with a `BUSINESS_TYPES` slug (e.g. `"bridal"`) already
returns the right businesses — this function does not need to change for
Phase 2.2 to work. This confirms the missing piece really was only the
homepage section (2.2.b) + route (2.2.d), not the data layer. **Checklist:
open the file, diff it mentally against the block above, and if it
matches, move on — do not add a new function.**
...
2. **Fully automated later:** once ADNAVRA has a verified WhatsApp Business
   ...
   gracefully if not configured — same shape as the `resend` check above:
   create the client once at module scope only if the env var is set,
   then check for `null` before every use instead of throwing:
   ```ts
   // src/lib/notify.ts — add alongside the existing `resend` line
   ...
   // inside notifyCustomerOfDecision, after the resend.emails.send(...) block:
   if (twilioClient && booking.business.phone) {
     await twilioClient.messages.create({
       from: `whatsapp:${process.env.TWILIO_WHATSAPP_FROM}`, // e.g. "whatsapp:+14155238886" (Twilio sandbox) or your approved number
       to: `whatsapp:${booking.business.phone}`,
       body: `Your booking (ref ${booking.reference}) at ${booking.business.name} has been ${decided}.`,
     });
   }
   ```
   Add `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_WHATSAPP_FROM` to
   `.env`/`.env.example`/Vercel env vars when you're ready to switch this on
   — until then, `twilioClient` stays `null` and this block is silently
   skipped, exactly like the `resend` block above skips when
...
**Edit `src/app/[businessSlug]/page.tsx`.** Today the file always renders
both an empty-state block *and* the real grid depending on a length check
inline — the exact current code (around line 564–588) is:
```tsx
    ...
    <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
      {business.staffMembers.map((m) => (
        <div key={m.id} className={`${CARD} flex items-center gap-4 p-4`}>
          <div className={`${SERIF} flex h-14 w-14 shrink-0 items-center justify-center rounded-full border border-[#E4D6BF] bg-[#F7F1E4] text-xl font-semibold text-[#795831]`}>
            {initials(m.name)}
          </div>
          <div className="min-w-0">
...
```
This is what's producing image 4's always-visible dashed empty card. Your
instruction was to hide the **entire section** (not show an empty-state
card) when there's no data, so replace the whole block above with:
```tsx
{/* Team — only rendered when the salon actually has staff */}
{business.staffMembers.length > 0 && (
  <section aria-label="Team">
    <SectionHeader id="specialists" eyebrow="Artistry & care" title="Meet the Team" />
    <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
...
```
The card markup inside the `.map()` is untouched, word for word — only the
outer wrapper changed from an "empty-state vs. grid" ternary to "render
nothing vs. render the section".
```tsx
...
This is the **entire, current, real content** of
`src/components/business/venue-gallery.tsx` (56 lines, copied verbatim —
your agent should open the file and confirm it matches before editing):
```tsx
"use client";
...
```
The one `{photos.length > 1 && (<div className="mt-3 grid grid-cols-3
gap-3 sm:grid-cols-4" ...>{photos.map(...)}</div>)}` block (lines 32–53
above) is what renders **all** photos at every breakpoint — that's the bug.
**Replace that one block** with two blocks: a mobile-only one sliced to 3,
and a desktop-only one with the full set. Nothing else in the file changes
— same imports, same `useState`, same featured-image block above it.
```tsx
{photos.length > 1 && (
...
```
Every line inside each `<button>` is identical between the two blocks —
only the wrapping `<div>`'s classes (`sm:hidden` + `.slice(0, 3)` vs.
`hidden sm:grid` + full `photos`) differ. Duplicating the button markup
into both blocks (rather than one shared block with a responsive slice) is
intentional — it keeps the two breakpoints fully independent and avoids a
server/client hydration mismatch from conditionally slicing an array
differently per render.

## 5.6 — Always show 4 treatments, add "See all" for the rest
`src/components/business/service-tabs.tsx` currently renders **every**
service in the active tab with no cap (159 lines total in the real file).
The relevant current block (lines 60–61 and 105–155) is:
```tsx
...
// ...
  ...
  <ul className="mt-5 grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-3 2xl:grid-cols-4">
    {visible.map((s) => {
      ...
      return (
        <li key={s.id} className="group flex flex-col overflow-hidden rounded-2xl border border-[#E9E1D3] bg-white shadow-[0_1px_3px_rgba(30,28,26,0.04)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_10px_28px_rgba(30,28,26,0.10)]">
          <Link href={href} className="relative block aspect-[4/3] overflow-hidden bg-[#F1E9DC]" tabIndex={-1} aria-hidden="true">
            <ServiceImage name={s.name} category={s.category} imageUrl={s.imageUrl} className="h-full w-full transition-transform duration-500 group-hover:scale-105" />
            {s.category && (
          ...
          <div className="flex flex-1 flex-col p-3.5 sm:p-4">
            <h3 className="line-clamp-2 text-lg font-medium leading-snug text-[#1F1B17] sm:text-xl" style={SERIF}>{s.name}</h3>
            {s.description && (
              <p className="mt-1.5 hidden line-clamp-3 text-xs leading-relaxed text-[#7A7368] sm:block">{s.description}</p>
            )}
            <div className="mt-auto flex items-end justify-between gap-2 pt-4">
              <div className="min-w-0">
                <p className="flex items-center gap-1 text-[11px] text-[#8A8377]"><Clock className="h-3 w-3" /> {s.duration} min</p>
                <p className="mt-0.5 text-sm font-semibold text-[#1F1B17] sm:text-base">{formatPrice(s.price)}</p>
              </div>
              <Link href={href} className="inline-flex h-8 shrink-0 items-center rounded-full bg-[#F3EEE4] px-4 text-[10px] font-bold uppercase tracking-[0.16em] text-[#5F4426] transition-colors hover:bg-[#1F1B17] hover:text-white">
                Book
              </Link>
            </div>
          </div>
...
```
Make three small, surgical changes to this file — everything inside the
`<li>` stays exactly as shown above, word for word:
1. Add `useEffect` to the existing `import { useMemo, useState } from
   "react";` line, so it reads `import { useMemo, useState, useEffect }
   from "react";`.
2. Right after the existing `const visible = ...` line, add:
   ```tsx
   const [showAll, setShowAll] = useState(false);
   const visibleServices = showAll ? visible : visible.slice(0, 4);
   useEffect(() => setShowAll(false), [active]); // reset the cap whenever the tab changes
   ```
3. In the `<ul>` block quoted above, change **only** `{visible.map((s) =>`
   to `{visibleServices.map((s) =>` (the entire `<li>...</li>` body inside
   stays identical), and add this directly after the closing `</ul>`:
   ```tsx
...
```
**Wire it into the wizard**, `src/app/dashboard/onboarding/page.tsx`. This
file does **not** use a `switch` — it's a chain of `if (step === N) return
(...)` blocks, one per step, each rendering a `<WizardShell>`. This is the
**entire real file, 167 lines, verbatim** (read it before editing so the
diff below makes sense in place):
```tsx
"use client";
import { useState } from "react";
import { useRouter } from "next/navigation";
...
export default function OnboardingPage() {
  ...
  const [location, setLocation] = useState<LocationFields>({
    address: "", district: "", city: "", county: "", state: "", postcode: "",
    directions: "", latitude: 6.9271, longitude: 79.8612,
  });
    ...
    try {
      const res = await fetch("/api/businesses", {
        ...
        body: JSON.stringify({
          ...
          categories, teamSize, locationType,
  ...
  if (step === 1) {
    return (
      <WizardShell step={1} totalSteps={TOTAL_STEPS} title="What's your business name?"
        subtitle="This is the brand name your clients will see. Your billing and legal name can be added later."
        onNext={() => setStep(2)} nextDisabled={!name.trim()}>
  ...
  if (step === 2) {
    return (
      <WizardShell step={2} totalSteps={TOTAL_STEPS} title="Select categories that best describe your business"
        subtitle="Choose your primary and up to three related service types."
        onBack={() => setStep(1)} onNext={() => setStep(3)} nextDisabled={categories.length === 0}>
  ...
  if (step === 3) {
    return (
      <WizardShell step={3} totalSteps={TOTAL_STEPS} title="What's your team size?"
        subtitle="This will help us set up your calendar correctly."
        onBack={() => setStep(2)} onNext={() => setStep(4)} nextDisabled={!teamSize}>
  ...
  if (step === 4) {
    return (
      <WizardShell step={4} totalSteps={TOTAL_STEPS} title="Where do you provide your services?"
  ...
  return (
    <WizardShell step={5} totalSteps={TOTAL_STEPS} title="Set your venue's physical location"
      subtitle="Add your primary business location so your clients can easily find you."
      onBack={() => setStep(4)} onNext={handleFinish}
      nextLabel={saving ? "Saving..." : "Finish"} nextDisabled={saving}>
...
```
The new salon-type step is inserted as **step 3**, pushing team size to 4,
location type to 5, and the map to 6. Below is the **complete replacement
file** — copy this over the whole file rather than trying to patch pieces,
since almost every step's number and `onBack`/`onNext` target shifts by
one:
```tsx
"use client";
import { useState } from "react";
...
import { StepBusinessName } from "@/components/onboarding/step-business-name";
import { StepCategories } from "@/components/onboarding/step-categories";
import { StepSalonTypes } from "@/components/onboarding/step-salon-types"; // NEW
import { StepTeamSize } from "@/components/onboarding/step-team-size";
import { StepLocationType } from "@/components/onboarding/step-location-type";
import { StepLocationMap, type LocationFields } from "@/components/onboarding/step-location-map";
...
export default function OnboardingPage() {
  ...
  const [website, setWebsite] = useState("");
  const [categories, setCategories] = useState<string[]>([]);
  const [salonTypes, setSalonTypes] = useState<string[]>([]); // NEW
  const [teamSize, setTeamSize] = useState("");
  const [locationType, setLocationType] = useState("PHYSICAL");
  const [location, setLocation] = useState<LocationFields>({
    address: "", district: "", city: "", county: "", state: "", postcode: "",
  ...
  function toggleSalonType(id: string) { // NEW
  ...
  if (step === 3) { // NEW STEP — salon type
    return (
      <WizardShell step={3} totalSteps={TOTAL_STEPS} title="What type of salon are you?"
        subtitle="Choose up to 4 — e.g. Gents, Ladies, Unisex, Bridal, Home visits."
        onBack={() => setStep(2)} onNext={() => setStep(4)} nextDisabled={salonTypes.length === 0}>
        <StepSalonTypes selected={salonTypes} onToggle={toggleSalonType} />
      </WizardShell>
    );
  }
  if (step === 4) { // was step 3
    return (
      <WizardShell step={4} totalSteps={TOTAL_STEPS} title="What's your team size?"
        subtitle="This will help us set up your calendar correctly."
        onBack={() => setStep(3)} onNext={() => setStep(5)} nextDisabled={!teamSize}>
        <StepTeamSize value={teamSize} onChange={setTeamSize} />
      </WizardShell>
    );
  }
  if (step === 5) { // was step 4
    return (
      <WizardShell step={5} totalSteps={TOTAL_STEPS} title="Where do you provide your services?"
  ...
  // step 6 — was step 5
  return (
    <WizardShell step={6} totalSteps={TOTAL_STEPS} title="Set your venue's physical location"
      subtitle="Add your primary business location so your clients can easily find you."
      onBack={() => setStep(5)} onNext={handleFinish}
      nextLabel={saving ? "Saving..." : "Finish"} nextDisabled={saving}>
      <StepLocationMap value={location} onChange={setLocation} />
      {error && <p className="mt-4 text-sm text-[#F87171]">{error}</p>}
    </WizardShell>
...
becomes
```
src/app/(onboarding)/dashboard/onboarding/layout.tsx   <-- NEW, minimal (full content in 6.4 below)
src/app/(onboarding)/dashboard/onboarding/page.tsx      <-- MOVED file, content is the FULL 6-step file from Task 6.2 above (with StepSalonTypes wired in as step 3) — not the original 5-step version
```
Concretely, do these three things in order:
1. Create the new folder path `src/app/(onboarding)/dashboard/onboarding/`.
2. Create `page.tsx` inside it and paste the **complete 6-step
   `OnboardingPage` component from Task 6.2 above** (the one with
   `StepSalonTypes` as step 3 and `TOTAL_STEPS = 6`) — that is the final
   content this file should have, already incorporating both changes
   (the new step **and** the move) in one file, so you don't have to apply
   6.2's edit twice.
3. Delete the old file at `src/app/dashboard/onboarding/page.tsx` so only
   one `page.tsx` resolves to the `/dashboard/onboarding` URL.
...
This is the **real, current, complete query** in
`src/app/admin/businesses/page.tsx` (lines 15–54) — open the file and
...
```
`orderBy: { createdAt: "desc" }` is already correct — **do not touch that
line**. The one real gap: `take: 100` with no pagination, so once you have
more than 100 salons some will silently stop appearing. Replace the
`const { q } = ...` line down through the `findMany` call above with:
```tsx
...
const [businesses, total] = await Promise.all([
  db.business.findMany({
    ...
    select: {
      ...
      createdAt: true,
      subscription: { select: { plan: true, status: true } },
      users: { where: { role: "OWNER" }, select: { email: true }, take: 3 },
    },
  }),
  db.business.count({ where }),
]);
```
Also update the function's type signature so `page` is a valid search
param: `searchParams: Promise<{ q?: string; page?: string }>`. Then add
simple Prev/Next links at the bottom of the table:
```tsx
<div className="mt-6 flex items-center justify-between text-sm">
  <span className="text-[#8A8377]">Page {page} of {Math.max(1, Math.ceil(total / PAGE_SIZE))} · {total} total</span>
  <div className="flex gap-2">
    {page > 1 && (
      <Link href={`/admin/businesses?${new URLSearchParams({ ...(query ? { q: query } : {}), page: String(page - 1) })}`} className="rounded-full border border-[#E5DDD0] px-4 py-2 font-medium hover:bg-[#F7F3ED]">Prev</Link>
    )}
    {page * PAGE_SIZE < total && (
      <Link href={`/admin/businesses?${new URLSearchParams({ ...(query ? { q: query } : {}), page: String(page + 1) })}`} className="rounded-full border border-[#E5DDD0] px-4 py-2 font-medium hover:bg-[#F7F3ED]">Next</Link>
    )}
  </div>
</div>
```
...
This is the complete, real, current file (`src/components/customer/home/trust-stats-bar.tsx`, 38 lines) — the piece your agent needs is the exact `title` expression on the line marked below, which must be carried over unchanged into the new markup:
```tsx
import { ShieldCheck, BadgePercent, Mail } from "lucide-react";
import { Reveal } from "./reveal";

const STATS = [
  { icon: ShieldCheck, title: "Verified Salons & Spas", desc: "Audited hygiene & master credentials" },
  { icon: BadgePercent, title: "Zero Booking Fee", desc: "Direct salon rates with zero card surcharges" },
  { icon: Mail, title: "Instant Email Confirm", desc: "Calendar-ready reminders with location maps" },
] as const;

export function TrustStatsBar({ businessCount }: { businessCount: number }) {
  return (
    <section className="relative bg-transparent">
      <Reveal as="div" className="max-w-[1400px] mx-auto px-6 lg:px-12 py-6 grid gap-6 sm:grid-cols-3 reveal-stagger">
        {STATS.map((s) => (
          <div key={s.title} className="flex items-center gap-3">
            ...
            <div>
              {/* KEEP THIS EXACT EXPRESSION — it prefixes the live business count onto only the "Verified…" stat */}
...
```
Replace the whole file with this version — same imports, same `STATS`
array, same `title` expression copied exactly as shown (do not simplify or
rewrite it, it's what makes the business count appear only on the first
card) — only the JSX inside the `.map()` changes, to this:
```tsx
{STATS.map((s) => (
  <div key={s.title} className="flex items-center gap-3 rounded-xl border border-[#E5DDD0]/60 bg-white/60 px-4 py-3 sm:border-0 sm:bg-transparent sm:px-0 sm:py-0">
    ...
    <div className="min-w-0">
      <p className="text-sm font-semibold text-[#1F1E1D] truncate">
        {s.title.startsWith("Verified") ? `${businessCount > 0 ? `${businessCount}+ ` : ""}${s.title}` : s.title}
      </p>
...
```
And the outer `<Reveal>` wrapper's `className` changes from
`"max-w-[1400px] mx-auto px-6 lg:px-12 py-6 grid gap-6 sm:grid-cols-3 reveal-stagger"`
to
`"max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 py-5 sm:py-6 grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-3 reveal-stagger"`
(adds mobile padding/gap tightening and forces single-column stacking below
`sm:`, which is what gives each stat its own bordered row on a phone).
